...
(() => {
   ...
   * rectangle and is never moved. Each box covers the printed extent of one
   * statement or display, so a labelled display row inside a theorem is now
   * NESTED inside that theorem's box by design - that is the mathematics, not
   * a collision, and neither box is trimmed for it. The outer box is marked
   * (.has-inner) so it can be painted a shade lighter, and --depth keeps the
   * smaller box above the larger one so the inner row stays clickable.
   * Genuine SIBLING overlaps - two rows whose recorded rectangles cross
   * because the printed rows themselves cross - are still resolved for DISPLAY
   * ONLY: the upper box is clipped back to the lower box's top edge (or, for
   * two boxes on one line, to the next box's left edge). clip-path also clips
   * hit-testing, so both the painted region and the click target become
   * disjoint, while the element's border box - which a regression test compares
   * against the stored coordinates - is left exactly as recorded. A trim never
   * removes more than 82% of a box, so every anchor keeps a visible,
   * clickable sliver. */
  const OVERLAP_EPS=0.004,OVERLAP_GAP=0.002,MIN_VISIBLE=0.18,NESTED=0.9;
  function separateHighlights(boxes) {
    ...
    /* b sits inside a: a is the larger box and holds almost all of b. */
    const holds=(a,b)=>area(a)>area(b)&&shared(a,b)>=NESTED*area(b);
    for(const box of boxes){box.cutBottom=box.rect.y1;box.cutRight=box.rect.x1;box.outer=false;}
    for(const a of boxes)for(const b of boxes){
      if(a===b||!meet(a.rect,b.rect))continue;
      if(holds(a.rect,b.rect)){a.outer=true;continue;}
      if(holds(b.rect,a.rect))continue;
    ...
    [...boxes].sort((p,q)=>area(q.rect)-area(p.rect)).forEach((box,index)=>box.button.style.setProperty('--depth',String(2+Math.min(index,120))));
    for(const box of boxes){
      box.button.classList.toggle('has-inner',box.outer);
  ...
  function renderDetail(anchor) {
    ...
    host.append(el('p','trace-tooltip','Highlights box the printed extent of a labelled statement or display, from its header to its last line on that page; a statement continuing onto the next page is cut at the page end. They do not delimit or certify proofs. A labelled display inside a statement sits nested inside that statement\'s box; where two sibling boxes overlap, the upper one is trimmed for display and the recorded coordinates are unchanged. Paper-to-Lean correspondence is curated; compiler-derived references remain available in the Lean view.'));
