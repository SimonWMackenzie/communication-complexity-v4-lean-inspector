...
(() => {
  ...
  async function start(event){
    ...
    const bar=el('div','graph-statusbar'),count=el('div');count.id='graph-count';count.setAttribute('role','status');bar.append(count);const zoomText=el('span');zoomText.id='graph-zoom';bar.append(btn('−',()=>{zoom=Math.max(.3,zoom-.1);applyZoom();}),zoomText,btn('+',()=>{zoom=Math.min(2.5,zoom+.1);applyZoom();}),btn('Fit width',fit),btn('Export this graph',download));main.append(bar);
